...
public class Ball {
    ...
    public void blockMove(Rect blockFrame){
        if (blockFrame.contains(point.x, point.y)
                || blockFrame.contains(point.x + size.x, point.y)
                || blockFrame.contains(point.x, point.y + size.y)
                || blockFrame.contains(point.x + size.x, point.y + size.y)) {
            delta.x *= -1;
            delta.y *= -1;
        }
    }
